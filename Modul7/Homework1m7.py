...
byron = 'Byron.txt'  # Создаем переменную с этим файлом
file = open(byron, mode='r', encoding='utf8')
for line in file:
    print(line, end='')  # Распечатываем содержимое текстового файла в консоль
file.close()

print('\n', 'ИСПОЛЬЗОВАНИЕ ОПЕРАТОРА WITH', '\n')

byron1 = 'Byron.txt'  # Создаем новую переменную с этим файлом
with open(byron1, mode='r', encoding='utf8') as file:
    for line in file:
        print(line, end='')  # Распечатываем содержимое текстового файла в консоль, используя оператор with
print(file.closed)  # Проверяем действительность закрытия файла

# Так можно вызвать любой файл:
byron = r"C:\Users\Александр\Homeworks\Modul7\Byron.txt"  # Создаем переменную с адресом файла
with open('Byron.txt', 'r') as file:
    text = " ".join(line.rstrip() for line in file)
    print(text)  # Распечатываем содержимое текстового файла в консоль

print(file.closed)  # Проверяем действительность закрытия файла
print('\n', 'Оператор "with open(file_name...)", в отличие от простого использования file.close()'
            'работает с момента открытия файла и гарантировано закрывает его, после необходимой обработки')
